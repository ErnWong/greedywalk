function [testCount, okayCount] = TestBestPathVectorized()
	...
	disp('# .--------------------.');
	disp('# | Testing BestPathVectorized.m |');
	disp('# `--------------------`');
	...
	if true
				  ...
				  for e3 = 1:k
					for e2 = 1:k
					  for e1 = 1:k
						...
						[pathRows, ~, elev] = BestPathVectorized(E);
	  ...
	  fprintf('# BestPathVectorized completed in %f seconds\n', toc);
	  if ~hasErrored
	...
	okayCount = okayCount + TestFunction(@BestPathVectorized, 2, {E2}, {[1 1 1 1 1 1 1 1], [1 2 3 4 5 6 7 8], [3 5 5 4 1 3 5 1]});

	E3 = [
	  1 3 4;
	  8 7 6
	];
	okayCount = okayCount + TestFunction(@BestPathVectorized, 3, {E3}, {[2 2 2], [1 2 3], [8 7 6]});

	E4 = [
	  1 2;
	  2 2
	];
	okayCount = okayCount + TestFunction(@BestPathVectorized, 4, {E4}, {[2 1], [1 2], [2 2]});
	...
	okayCount = okayCount + TestFunction(@BestPathVectorized, 5, {E5}, {[1], [1], [1]});

	E6 = [
	  2 4 3;
	  5 3 1;
	];
	okayCount = okayCount + TestFunction(@BestPathVectorized, 6, {E6}, {[1 2 1], [1 2 3], [2 3 3]});
	...
	okayCount = okayCount + TestFunction(@BestPathVectorized, 7, {E7}, {[3 2 1], [1 2 3], [1 2 1]});
	...
	okayCount = okayCount + TestFunction(@BestPathVectorized, 8, {E8}, {[5 5 5 6], [1 2 3 4], [3 3 3 3]});
	...
	okayCount = okayCount + TestFunction(@BestPathVectorized, 9, {E9}, {[1 2 3 4], [1 2 3 4], [1 1 0 2]});

	for i = 1:3
		fprintf('# Performing 10x10 randomised test %d\n', i);
		disp('# (1) Generating elevations');
		E10 = randi(1000, 10);
		disp('# (2) Running BestPathVectorized');
		[~, ~, elev] = BestPathVectorized(E10);
		daCost = sum(abs(elev(2:10) - elev(1:9)));
		fprintf('# BestPathVectorized cost: %d\n', daCost);
	...
	for i = 1:4
		...
		disp('# (2) Running BestPathVectorized');
		tic;
		BestPathVectorized(E);
		t = toc;
		fprintf('# ==> BestPathVectorized took %f seconds\n', t);
		if t < 10
			okayCount = okayCount + 1;
			fprintf('ok %d\n', 12 + i);
		else
	...
	disp('# Computing Path');
	tic;
	[pathRow, pathCol, pathElev] = BestPathVectorized(E);
	fprintf('# BestPathVectorized completed in %f seconds\n', toc);
	...
	fprintf('\n# TestBestPathVectorized - %d / %d tests passed\n', okayCount, testCount);
	disp('# TestBestPathVectorized - Done');
	fprintf('\n\n');
